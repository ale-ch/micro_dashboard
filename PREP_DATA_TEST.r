...
sample_municipalities <- function(df, seed=seed_number, n=n_samples, sample=TRUE) {
...
sample_municipalities_procom <- function(df, seed=seed_number, n=n_samples, sample=TRUE) {
  if(isFALSE(sample)) {
    return(df)
  } else {
    set.seed(seed)

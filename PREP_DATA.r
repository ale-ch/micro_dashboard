...
prep_data <- function(data, REF_AREA, TIME_PERIOD) {
  ...
  data %>%
    ...
    mutate(
      PRO_COM_T = stringr::str_pad(PRO_COM_T, width = 6, side = "left", pad = "0")
    )
}
...
sample_municipalities <- function(df, seed=seed_number, n=n_samples, sample=FALSE) {
...
sample_municipalities_procom <- function(df, seed=seed_number, n=n_samples, sample=FALSE) {
  if(isFALSE(sample)) {
    return(df)
  } else {
    set.seed(seed)
    sampled_comuni <- sample(comuni$PRO_COM, n)
